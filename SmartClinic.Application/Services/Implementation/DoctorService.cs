...
public class DoctorService : IDoctorService
{
    ...
    public async Task<Response<CreateDoctorResponse>> CreateDoctor(CreateDoctorRequest newDoctorUser)
    {
        ...
        {
            return new ResponseHandler().BadRequest<CreateDoctorResponse>(errors: new List<string> { "No image uploaded" });
        }

        var validationOptions = new FileValidation
        {
            MaxSize = 2 * 1024 * 1024,
            AllowedExtenstions = new List<string> { ".jpg", ".jpeg", ".png" }
        };
        ...
        if (newDoctorUser.Specialization != null && newDoctorUser.Specialization.Any())
        {
            foreach (var specializationId in newDoctorUser.Specialization)
            {
                var specializationResponse = await _specializationService.GetSpecializationByIdAsync(specializationId);

                if (specializationResponse != null && specializationResponse.Data != null)
                {
    ...
    public async Task<Response<UpdateDoctorResponse>> UpdateDoctorAsync(int doctorId, UpdateDoctorRequest request)
    {
        ...
        if (request.Specialization != null && request.Specialization.Any())
        {
            var specializations = new List<Specialization>();

            foreach (var specializationId in request.Specialization)
            {
                var specializationResponse = await _specializationService.GetSpecializationByIdAsync(specializationId);
                if (specializationResponse?.Data != null)
                {
                    specializations.Add(specializationResponse.Data);
                }
            }
