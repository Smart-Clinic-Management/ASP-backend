...
namespace SmartClinic.Application.Features.Auth.NewFolder
{
    internal class AuthService : IAuthService
    {
        ...
        public async Task<Response<LoginResponseDTO>> Login(LoginRequestDTO user)
        {
            ...
            if (userExist == null)
                return response.NotFound<LoginResponseDTO>();

            var sign = await signMGR.CheckPasswordSignInAsync(userExist, user.Password, false);

            if (!sign.Succeeded)
                return response.BadRequest<LoginResponseDTO>("invalid login attempt")!;
